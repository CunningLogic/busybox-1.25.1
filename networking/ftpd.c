...
#if ENABLE_FEATURE_DJI_FTPD_FILE_ENCRYPT
...
#if ENABLE_FEATURE_DJI_FTPD_FILE_ENCRYPT
static void enc_file(int src_fd, int dest_fd, off_t size, bool enc_or_dec)
{
	#define BUFFER_SIZE 4096
	unsigned int bytes_rd, bytes_wr;
	off_t total_bytes_rd = 0, total_bytes_wr = 0;
	uint8_t rd_buf[BUFFER_SIZE];

	uint8_t key[KEYBYTES];
	uint8_t iv[AES_BLOCK_SIZE];

	uint8_t enc_buf[BUFFER_SIZE + 16];
	int enc_len;

	struct aes_ctx *ctx;

	memset(key, 0, KEYBYTES);
	strncpy((char *)key, getenv("AES_KEY"), KEYBYTES);
	memset(iv, 0, AES_BLOCK_SIZE);
	strncpy((char *)iv, getenv("AES_IV"), AES_BLOCK_SIZE);

	ctx = aes_alloc(key, KEYBITS, iv);
	if (!ctx)
		return;

	while ((bytes_rd = read(src_fd, rd_buf, BUFFER_SIZE)) >= 0) {
		bool is_last = false;

		total_bytes_rd += bytes_rd;

		if (total_bytes_rd == size)
			is_last = true;
		else
			is_last = false;

		if (enc_or_dec) {
			enc_len = aes_cbc_encrypt(ctx, rd_buf, bytes_rd,
				enc_buf, sizeof(enc_buf), is_last);
		} else {
			enc_len = aes_cbc_decrypt(ctx, rd_buf, bytes_rd,
				enc_buf, sizeof(enc_buf), is_last);
		}

		while ((bytes_wr = write(dest_fd, enc_buf, enc_len)) >= 0) {
			total_bytes_wr += bytes_wr;
...
#if ENABLE_FEATURE_DJI_FTPD_FILE_ENCRYPT_DEBUG
	fprintf(stderr, "total read %llu, total write %llu\n", total_bytes_rd, total_bytes_wr);
#endif
}
#endif
...
#if ENABLE_FEATURE_DJI_FTPD_FILE_ENCRYPT
	if (strncmp(G.ftp_arg, "/flyctrl", 8)) {
		sprintf(tmp_file_name, "%s.tmp", G.ftp_arg);
		tmp_fd = open(tmp_file_name, O_CREAT | O_RDWR, S_IRUSR|S_IWUSR);

		enc_file(local_file_fd, tmp_fd, statbuf.st_size, true);

		close(tmp_fd);
		close(local_file_fd);

		local_file_fd = open(tmp_file_name, O_RDONLY | O_NONBLOCK);
	}
#endif
...
#if ENABLE_FEATURE_DJI_FTPD_FILE_ENCRYPT_DEBUG
	if (strncmp(G.ftp_arg, "/flyctrl", 8)) {
		sprintf(tmp_file_name, "%s.dec", G.ftp_arg);
		tmp_fd = open(tmp_file_name, O_CREAT | O_RDWR, S_IRUSR|S_IWUSR);

		sprintf(tmp_file_name, "%s.tmp", G.ftp_arg);
		local_file_fd = open(tmp_file_name, O_RDONLY | O_NONBLOCK);
		fstat(local_file_fd, &statbuf);

		enc_file(local_file_fd, tmp_fd, statbuf.st_size, false);
...
#if ENABLE_FEATURE_DJI_FTPD_FILE_ENCRYPT
/* encrypt algorithm will generate 16 bytes aligned files*/
#define FILE_LEN_ALIGN 16
/* line[27] is the start of file len in the string */
#define LEN_STRING_START 27
/* the max file len in decimal format is 16 in the string */
#define MAX_FILE_LEN 16

static void DJI_file_len_align(char *line, int align)
{
	/* file length is possible to > 4GB, so use a 64 bit variable */
	unsigned long long file_len;
	char buf[20];
	int i;

	/* get the original file len from the string */
	sscanf(&line[LEN_STRING_START], "%llu", &file_len);
	/* make the lenght to be 16 bytes aligned */
	sprintf(buf, "%16llu", file_len + align - (file_len % align));
	/* put the characters back to original string */
	for(i = 0; i < MAX_FILE_LEN; i++)
		line[i + LEN_STRING_START] = buf[i];
}
#endif
...
#if ENABLE_FEATURE_DJI_FTPD_FILE_ENCRYPT
				 /* line[0] == '-' means this is a file, 'd' means this is a folder */
				if(line[0] == '-') {

#if ENABLE_FEATURE_DJI_FTPD_FILE_ENCRYPT_DEBUG
					fprintf(stderr, "before file align: %d %s\n", len, line);
#endif
					DJI_file_len_align(line, FILE_LEN_ALIGN);

#if ENABLE_FEATURE_DJI_FTPD_FILE_ENCRYPT_DEBUG
					fprintf(stderr, "after  file align: %d %s\n", len, line);
#endif
				}
#endif
